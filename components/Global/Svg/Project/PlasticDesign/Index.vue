<template>
  <div
    :class="[
      'svg__project__plastic-design',
      { 'svg__project__plastic-design--animation': animation },
    ]">
    <template v-if="isMobileLayout">
      <Ticker
        ...
        class="svg__project__plastic-design__ticker"
        @update="emit('update-scroll')">
        <div v-for="i in 2" :key="i">
          <SvgProjectPlasticDesignFirstLine
            v-transition:in="{ callback: animation ? shuffleIn : () => {} }" />
        </div>
      </Ticker>
      <SvgProjectPlasticDesignSecondLine
        v-transition:in="{ callback: animation ? shuffleIn : () => {} }" />
    </template>
    <template v-else>
      <SvgProjectPlasticDesignFirstLine
...
const id = 'project-ticker-plastic-design'
