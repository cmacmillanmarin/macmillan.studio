<template>
  <div :class="['svg__project__h2o', { 'svg__project__h2o--animation': animation }]">
    <template v-if="isMobileLayout">
      <Ticker
        ...
        class="svg__project__h2o__ticker"
        @update="emit('update-scroll')">
        <div v-for="i in 2" :key="i">
          <SvgProjectH2oFirstLine
            v-transition:in="{ callback: animation ? shuffleIn : () => {} }" />
        </div>
      </Ticker>
    </template>
    <template v-else>
      <SvgProjectH2oFirstLine v-transition:in="{ callback: animation ? shuffleIn : () => {} }" />
    </template>
  </div>
</template>
