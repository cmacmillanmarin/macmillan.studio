<template>
  <div
    :class="['svg__project__orbit-viewer', { 'svg__project__orbit-viewer--animation': animation }]">
    <template v-if="isMobileLayout">
      <Ticker
        ...
        class="svg__project__orbit-viewer__ticker"
        @update="emit('update-scroll')">
        <div v-for="i in 2" :key="i">
          <SvgProjectOrbitViewerFirstLine
      ...
      <Ticker
        ...
        @update="emit('update-scroll')">
        <div v-for="i in 2" :key="i">
          <SvgProjectOrbitViewerSecondLine
            v-transition:in="{ callback: animation ? shuffleIn : () => {} }" />
        </div>
      </Ticker>
    </template>
...
const id = 'project-ticker-orbit-viewer'
