<template>
  <div
    :class="[
      'svg__project__pangea-propulsion',
      { 'svg__project__pangea-propulsion--animation': animation },
    ]">
    <template v-if="isMobileLayout">
      <SvgProjectPangeaPropulsionFirstLine
      ...
      <Ticker
        ...
        class="svg__project__pangea-propulsion__ticker"
        @update="emit('update-scroll')">
        <div v-for="i in 2" :key="i">
          <SvgProjectPangeaPropulsionSecondLine
            v-transition:in="{ callback: animation ? shuffleIn : () => {} }" />
        </div>
      </Ticker>
    </template>
...
const id = 'project-ticker-pangea-propulsion'
