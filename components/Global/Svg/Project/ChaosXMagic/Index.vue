<template>
  <div
    :class="[
      'svg__project__chaos-x-magic',
      { 'svg__project__chaos-x-magic--animation': animation },
    ]">
    <template v-if="isMobileLayout">
      <SvgProjectChaosXMagicFirstLine
      ...
      <Ticker
        ...
        class="svg__project__chaos-x-magic__ticker"
        @update="emit('update-scroll')">
        <div v-for="i in 2" :key="i">
          <SvgProjectChaosXMagicSecondLine
            v-transition:in="{ callback: animation ? shuffleIn : () => {} }" />
        </div>
      </Ticker>
    </template>
...
const id = 'project-ticker-chaos-x-magic'
